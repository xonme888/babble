...
대고객 대상이 아닌 현재는 일부 사용자를 통한 데이터 수집과 검증과정을 좀 더 후 대고객 오픈을 목표로 한다. 
...
8. [API 설계](#api-설계)
9. [게임화 시스템](#게임화-시스템)
10. [모바일 아키텍처 (비공개 요약)](#모바일-아키텍처-비공개-요약)
11. [트러블슈팅](#트러블슈팅)
12. [시작하기](#시작하기)
13. [공개 범위](#공개-범위)
14. [라이선스](#라이선스)
...
> **이 저장소는 포트폴리오 코드 열람 목적으로 공개되어 있습니다.** 공개된 4개 모듈(auth, assessment, gamification, user)과 shared 아키텍처만 포함되어 있으며, 실제 빌드 및 실행에는 비공개 모듈(game, learning, script, notification 등)이 필요합니다. 테스트는 공개 모듈 범위 내에서 독립 실행 가능합니다.
