...
| 영역 | 내용 |
|------|------|
| **아키텍처** | 헥사고날 아키텍처 + Port 기반 DI (11개 인터페이스) + Domain Event 부수효과 분리 |
| **인증** | JWT 이중 토큰 + Token Rotation + 10초 Grace Period (race condition 방지) |
| **비동기 처리** | 음성 업로드 → BullMQ 큐 → AI Worker → Redis Pub/Sub → SSE 실시간 전송 |
...
| **테스트** | Unit / Integration / E2E 3단계 테스트 + OpenAPI 계약 검증 |
...
**핵심 규칙**: 의존성은 항상 안쪽으로. 피처 간 의존은 반드시 Port 인터페이스(11개)를 거치고, tsyringe가 런타임에 구현체를 주입한다. Domain Event로 핵심 로직과 부수 효과(XP, 뱃지, 학습 기록)를 분리했다.
...
| ORM | TypeORM | 데코레이터 기반 엔티티, Migration 지원 |
...
| 용어 | 정의 |
|------|------|
| **Assessment** | 사용자의 발음 평가 한 건. 음성 업로드 → AI 분석 → 점수/피드백 |
...
│   │   ├── domain/             # TokenRefreshPolicy, VerificationCode
│   │   ├── infrastructure/     # JwtTokenProvider, BcryptPasswordHasher
│   │   └── presentation/       # Routes, Guards (auth/admin/guest/voiceConsent)
│   ├── assessment/             # 발음 평가 (업로드, AI 분석, 결과) ★ 공개
│   ├── gamification/           # 게임화 (XP, 레벨, 뱃지, 설정) ★ 공개
│   ├── user/                   # 사용자 (프로필, 통계, 목표) — 비공개
...
├── shared/
│   ├── core/                   # Port 인터페이스, Base Entity, 예외, DI 토큰
│   ├── infra/                  # Config, Logger, DB, Redis, Queue, i18n
...
| 서비스 | 공개 | 비고 |
|--------|:----:|------|
| **app-backend** | 부분 공개 | 핵심 3개 모듈(auth, assessment, gamification) + shared 아키텍처 |
...
| 공개 모듈 | 보여주는 역량 |
|-----------|---------------|
| **auth** | JWT 이중 토큰 + Token Rotation + 분산 락, 게스트 체험 → 회원 병합 |
| **assessment** | BullMQ 비동기 처리 + Redis Pub/Sub + SSE 실시간 전송, 상태 머신 |
| **gamification** | Domain Event 기반 부수효과 분리, DB 주도 뱃지 시스템, XP/레벨 설계 |
| **shared** | 헥사고날 아키텍처, Port 인터페이스(11개), Base Entity, 공통 미들웨어 |
